...
while(True):
    ...
    if(displacement.response() > 0.1): # Below 0.1 or so (YMMV) and the results are just noise.
        f.write("{0:+f}".format(sub_pixel_x))
        #print("{0:+f}x {1:+f}y {2} {3} FPS".format(sub_pixel_x, sub_pixel_y,
              #displacement.response(),
              #clock.fps()))
        print("meters:", sub_pixel_x/365.714286)
        distance_total += sub_pixel_x/365.714286
        print("total meters:", distance_total)
    else:
        print(clock.fps())



#field of view: 15.5in from 1 foot away
