...
sigma_r = 5;  % [m] Measurement noise standard deviation
R = sigma_r^2;
...
for k = 2:numel(time)
    ...
    z_k(k) = H*x_true(:,k) + LR*randn(size(H,1),1);
    z_hat_k = H*x_hat_k(:,k); %Predicted measurement is our expected position
    y = z_k(k) - z_hat_k;
